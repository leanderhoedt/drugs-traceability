...
async function createDrug(drug) {
    ...
    let drugResource = factory.newResource('org.drugs', 'Drug', drug.drugHash);
    ...
    drugResource.manufacturer = factory.newRelationship(NS, 'Manufacturer', drug.manufacturer.getIdentifier());
 ...
 * @transaction
 */
async function updateDrugStatus(receiveDrugRequest) {
    ...
    if (receiveDrugRequest.customer && me.getFullyQualifiedIdentifier() != receiveDrugRequest.customer.getFullyQualifiedIdentifier()) {
        throw new Error('The customer that you signed does not match your identity!');
    }

    drug.owner = factory.newRelationship(NS, 'Drug', receiveDrugRequest.customer);
    if (receiveDrugRequest.distributer) {
        drug.distributer = receiveDrugRequest.distributer;
    }
    if (receiveDrugRequest.pharmacist) {
        drug.pharmacist = receiveDrugRequest.pharmacist;
    }
    ...
    receiveDrugEvent.drugTransactionStatus = updateDrugStatusRequest.drugTransactionStatus;
    receiveDrugEvent.drug = drug;
    receiveDrugEvent.customer = drug.owner;
...
async function verifyDrug(drug) {
    ...
    const originExists = drugRegistry.exists(drug.hash);
    if(!originExists){
